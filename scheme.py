...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        correct_procedure = scheme_eval(first, env)
        mapped = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(correct_procedure, mapped, env)
        # END PROBLEM 5
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if vals is not nil:
                ...
                raise SchemeError('number of arguments does not match number of parameters')
        if vals is not nil:
            raise SchemeError('number of arguments does not match number of parameters')
        return new_frame
        # END PROBLEM 11
...
def make_let_frame(bindings, env):
    ...
    while  bindings is not nil:
        ...
        evaluated = scheme_eval(bindings.first.second.first, env)
        formals = Pair(bindings.first.first, formals)
        vals = Pair(evaluated, vals)
